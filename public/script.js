...
var clrsfFire = ["#ff0000", "#f0430a", "#f06e0a"]
var clrsWater = ["#0aecf0", "#0abaf0", "#0a9ff0"]
    ...
    for (var y = 0; y < matrix.length; ++y) {
        for (var x = 0; x < matrix[y].length; ++x) {
            if (matrix[y][x] == 1) {
                ...
                var fire = new Fire(x, y, 6);
                FireArr.push(fire);
            }
            else if(matrix[y][x] == 7){
                var water = new Water(x, y, 7);
                WaterArr.push(water);
            }
        }
    }
...
function draw() {
    for (var y = 0; y < matrix.length; y++) {
        for (var x = 0; x < matrix[y].length; x++) {
            if (matrix[y][x] == 1) {
                ...
                fill(random(clrsfFire));
            } else if(matrix[y][x] == 7){
                fill(random(clrsWater));
            }
    ...
    if (FireArr.length == 0 && matrix[n][m] != 4) {
        var fire = new Fire(n, m, 6);
        FireArr.push(fire);
    }

    if(FireArr.length >= 30 && matrix[n][m] == 6){
        var water = new Water(n, m, 7);
        WaterArr.push(water);
    }
